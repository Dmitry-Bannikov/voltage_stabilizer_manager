...
#define I2C_CMD_ALIVE_INTERVAL_TICK    (30 / portTICK_PERIOD_MS)
#define I2C_CMD_EVT_ALIVE              (0)
#define I2C_CMD_EVT_DONE               (1)
#define I2C_EVT_QUEUE_LEN              (1)
#define I2C_SLAVE_TIMEOUT_DEFAULT      (1000)     /* I2C slave timeout value, APB clock cycle number */
...
has only 32 bytes.
*/
esp_err_t wire_driver_install(i2c_port_t i2c_num, i2c_mode_t mode, size_t slv_rx_buf_len, size_t slv_tx_buf_len,
                             int intr_alloc_flags)
{
    ...
    //hook isr handler
    wire_isr_register(i2c_num, i2c_isr_handler_default, p_i2c_obj[i2c_num], intr_alloc_flags, &p_i2c_obj[i2c_num]->intr_handle);
...
static esp_err_t i2c_master_clear_bus(i2c_port_t i2c_num)
{
    ...
    wire_set_pin(i2c_num, sda_io, scl_io, 1, 1, I2C_MODE_MASTER);
#else
...
esp_err_t wire_param_config(i2c_port_t i2c_num, const i2c_config_t *i2c_conf)
{
    ...
    ret = wire_set_pin(i2c_num, i2c_conf->sda_io_num, i2c_conf->scl_io_num,
...
{
    ESP_RETURN_ON_FALSE(i2c_num >= 0 && i2c_num < I2C_NUM_MAX && high_period != NULL && low_period != NULL, ESP_ERR_INVALID_ARG, I2C_TAG, I2C_NUM_ERROR_STR);
    I2C_ENTER_CRITICAL(&(i2c_context[i2c_num].spinlock));
...
{
    ESP_RETURN_ON_FALSE(i2c_num >= 0 && i2c_num < I2C_NUM_MAX && setup_time != NULL && hold_time != NULL, ESP_ERR_INVALID_ARG, I2C_TAG, I2C_NUM_ERROR_STR);
    I2C_ENTER_CRITICAL(&(i2c_context[i2c_num].spinlock));
...
{
    ESP_RETURN_ON_FALSE(i2c_num >= 0 && i2c_num < I2C_NUM_MAX && sample_time != NULL && hold_time != NULL, ESP_ERR_INVALID_ARG, I2C_TAG, I2C_NUM_ERROR_STR);
    I2C_ENTER_CRITICAL(&(i2c_context[i2c_num].spinlock));
...
{
    ESP_RETURN_ON_FALSE(i2c_num >= 0 && i2c_num < I2C_NUM_MAX && timeout != NULL, ESP_ERR_INVALID_ARG, I2C_TAG, I2C_NUM_ERROR_STR);
    i2c_hal_get_tout(&(i2c_context[i2c_num].hal), timeout);
    return ESP_OK;
}
...
esp_err_t wire_set_pin(i2c_port_t i2c_num, int sda_io_num, int scl_io_num, bool sda_pullup_en, bool scl_pullup_en, i2c_mode_t mode)
{
                                     ...
                                     TickType_t ticks_to_wait)
{
    ...
    i2c_cmd_handle_t handle = wire_cmd_link_create_static(buffer, sizeof(buffer));
    assert (handle != NULL);

    err = wire_master_start(handle);
    if (err != ESP_OK) {
        goto end;
    }

    err = wire_master_write_byte(handle, device_address << 1 | I2C_MASTER_WRITE, true);
    if (err != ESP_OK) {
        goto end;
    }

    err = wire_master_write(handle, write_buffer, write_size, true);
    if (err != ESP_OK) {
        goto end;
    }

    wire_master_stop(handle);
    err = wire_master_cmd_begin(i2c_num, handle, ticks_to_wait);

end:
    wire_cmd_link_delete_static(handle);
    return err;
}
                                      ...
                                      TickType_t ticks_to_wait)
{
    ...
    err = wire_master_write_byte(handle, device_address << 1 | I2C_MASTER_READ, true);
    if (err != ESP_OK) {
        goto end;
    }

    err = wire_master_read(handle, read_buffer, read_size, I2C_MASTER_LAST_NACK);
    if (err != ESP_OK) {
        goto end;
    }
...
esp_err_t wire_master_write(i2c_cmd_handle_t cmd_handle, const uint8_t *data, size_t data_len, bool ack_en)
{
    ...
    if (data_len == 1) {
         ...
         * correct this here. */
        return wire_master_write_byte(cmd_handle, *data, ack_en);
    }
...
esp_err_t wire_master_read(i2c_cmd_handle_t cmd_handle, uint8_t *data, size_t data_len, i2c_ack_type_t ack)
{
    ...
    if (ack != I2C_MASTER_LAST_NACK) {
        ...
        ret = wire_master_read_byte(cmd_handle, data, I2C_MASTER_NACK);
    } else {
        ...
        if (ret == ESP_OK) {
            ret = wire_master_read_byte(cmd_handle, data + data_len - 1, I2C_MASTER_NACK);
        }
    }
...
esp_err_t wire_master_cmd_begin(i2c_port_t i2c_num, i2c_cmd_handle_t cmd_handle, TickType_t ticks_to_wait)
{
    ...
    wire_reset_tx_fifo(i2c_num);
    wire_reset_rx_fifo(i2c_num);
